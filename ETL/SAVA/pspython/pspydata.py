...
class Measurement:
    def __init__(self, title, timestamp, 
                 current_arrays, potential_arrays, time_arrays, freq_arrays, 
                 zre_arrays, zim_arrays, 
                 aux_input_arrays, peaks, eis_fit, meas_type, 
...
class Curve:
    def __init__(self, title, measurement_type, x_array, y_array):
        self.Title = title
...
def convert_to_measurement(m, **kwargs):
    ...
    # if smooth_level > 0:
    #     for x in m:
    #         x.Smooth(smooth_level)
    
    notes = m.Method.Notes
    
    try:
        arrays = m.DataSet.GetDataArrays()
    except:
        print('Cannot write this measurement (unknown error), using timestamp from first curve')
        curves = m.GetCurveArray()
        timestamp = [_get_values_from_NETArray(curves[0].XAxisDataArray)]
        return Measurement(m.Title, m.TimeStamp.ToString(), 
                           None, None, timestamp, None, None, None, None, None, None, m.Method.ToString())
    ...
    for n, array in enumerate(arrays):
        ...
        if (array_type == ArrayType.Current):
            ...
            # # get the status of the meausured data pointei
    ...
    return Measurement(m.Title, m.TimeStamp.ToString(),
                       ...
                       peaks, eis_fits, m.Method.ToString(), notes=notes, method=m.Method)
...
def convert_to_curves(m):
    ...
    for c in curves_net:
        try:
            array_type = ArrayType(c.YArrayType)  # read measurement type
        except:
            array_type = ArrayType.Unspecified  # arraytype not implemented in ArrayType enum
            
        curve_ = Curve(c.Title, array_type.name, _get_values_from_NETArray(c.XAxisDataArray), _get_values_from_NETArray(c.YAxisDataArray))
...
def _get_values_from_DoubleArray(array, **kwargs):
    start = kwargs.get('start', 0)
    count = kwargs.get('count', array.Length)
    values = list()
    for i in range(start, start + count):
        value = array.get_Item(i)
        values.append(float(value.Value))
